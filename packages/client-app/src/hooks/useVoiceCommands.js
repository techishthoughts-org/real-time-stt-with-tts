...
export const useVoiceCommands = (config = {}) => {
    const defaultConfig = {
        enabled: true,
        sensitivity: 0.8,
        language: 'pt-BR',
    ...
    const processCommand = useCallback((transcript, confidence) => {
        if (confidence < defaultConfig.sensitivity)
            return;
    ...
    const calculateSimilarity = useCallback((transcript, phrase) => {
        const transcriptWords = transcript.split(' ');
        const phraseWords = phrase.toLowerCase().split(' ');
        let matches = 0;
        for (const word of transcriptWords) {
            if (phraseWords.some(phraseWord => word.includes(phraseWord) || phraseWord.includes(word))) {
                matches++;
            }
        }
    ...
    const getCommandsByCategory = useCallback((category) => {
        return Array.from(commandsRef.current.values())
