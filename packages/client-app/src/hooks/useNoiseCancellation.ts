...
interface AudioProcessor {
  ...
  updateNoiseProfile: (noiseSample: Float32Array) => void;
  reset: () => void;
}

export const useNoiseCancellation = (config: Partial<NoiseCancellationConfig> = {}) => {
  const defaultConfig: NoiseCancellationConfig = {
    enabled: true,
  ...
  const createNoiseProcessor = useCallback((): AudioProcessor => {
    ...
    const updateNoiseProfile = (noiseSample: Float32Array) => {
      if (!noiseProfile) {
  ...
  const initializeAudioContext = useCallback(async (stream: MediaStream) => {
    if (!defaultConfig.enabled) return stream;
  ...
  const calibrateNoise = useCallback(async (duration: number = 3000) => {
    if (!defaultConfig.enabled || !processorRef.current) return;
  ...
  const startNoiseCancellation = useCallback(async (stream: MediaStream) => {
    if (!defaultConfig.enabled) return stream;
