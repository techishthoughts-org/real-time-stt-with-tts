...
export class AudioCaptureService {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      console.log(
        '📊 Audio tracks:',
        this.mediaStream.getAudioTracks().map((track) => ({
          label: track.label,
  ...
  async startCapture(onAudioFrame: (frame: AudioFrame) => void): Promise<void> {
    ...
    this.processor.onaudioprocess = (event) => {
      ...
      frameCount++;
      if (isVoice) speechFrameCount++;

      // Log audio activity every 50 frames (about every second)
      if (frameCount % 50 === 0) {
        console.log(
          `📈 Audio stats: ${frameCount} frames processed, ${speechFrameCount} with speech, current RMS: ${rms.toFixed(4)}`
        );
      }
      ...
      if (isVoice) {
        console.log(
          `🗣️ SPEECH detected - Frame ${frame.seq}, RMS: ${rms.toFixed(4)}, Timestamp: ${frame.timestamp}`
        );
      }
